...
if(isset($user_result['hits']["hits"][0]["_source"]["save"]))
{
  $save=$user_result['hits']["hits"][0]["_source"]["save"];
...
if(isset($user_result['hits']["hits"][0]["_source"]["address"]))
  {
    $user_address=$user_result['hits']["hits"][0]["_source"]["address"];
    $user_latitude=$user_result['hits']["hits"][0]["_source"]["latitude"];
    $user_longitude=$user_result['hits']["hits"][0]["_source"]["longitude"];
  }
$param['index']='bird';
$param['body']['query']['term']['eventid']=$eventid;
$bird_result=$client->search($param);
$title=$bird_result['hits']["hits"][0]["_source"]["title"];
$imgSrc=$bird_result['hits']["hits"][0]["_source"]["imgSrc"];
$info=$bird_result['hits']["hits"][0]["_source"]["info"];
$price=$bird_result['hits']["hits"][0]["_source"]["price"];
$category=$bird_result['hits']["hits"][0]["_source"]["category"];
$time=$bird_result['hits']["hits"][0]["_source"]["time"];
$address=$bird_result['hits']["hits"][0]["_source"]["address"];
$latitude=$bird_result['hits']["hits"][0]["_source"]["latitude"];
$url=$bird_result['hits']["hits"][0]["_source"]["url"];
if($latitude!="0.0")
$longitude=$bird_result['hits']["hits"][0]["_source"]["longitude"];
?>
...
<div class="logo">
  ...
  <div class="event-detail mod">
      <div class="pic">
        <img  src="<?php echo "http://wjbianjason.eicp.net/showPic.php?picurl=".$imgSrc;?>" height="200" width="112" alt="活动海报">
      </div>
    <div class="info">
...
<script type="text/javascript">
  ...
  function save()
  {
    ...
    xmlhttp.open("POST","http://wjbianjason.eicp.net/save.php",true);
